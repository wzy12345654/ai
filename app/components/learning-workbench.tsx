...
export default function LearningWorkbench() {
  ...
  return <>
    ...
    <section ref={extractionRef} className={`extraction-card ${clip ? "available" : "disabled"}`} aria-labelledby="extract-title">
      <div className="section-title-row"><div><span className="section-kicker">STEP 02 · 当前开发阶段</span><h2 id="extract-title">提取英文内容</h2><p>从选区或完整音频中识别英文，你可以校对、复制并下载文本。</p></div><span className={`stage-badge ${extractionDone ? "complete" : ""}`}>{extractionDone ? "已完成 ✓" : clip ? "可开始" : "等待音频剪切"}</span></div>
      {!clip ? <div className="extract-locked"><span>02</span><strong>先在上方准备学习音频</strong><p>你可以导出选区，也可以跳过剪切直接使用完整音频。</p></div> : <>
        <div className="clip-summary"><div><span className="summary-icon">♫</span><div><small>准备识别的学习片段</small><strong>{clip.name}</strong><span>{clip.durationLabel} · {clip.formatLabel}</span></div></div><audio controls src={clip.url} /></div>
        {!extractionDone && <div className={`extract-start ${extracting ? "is-processing" : ""}`}><div className="extract-illustration">{extracting ? <i className="large-spinner" /> : <>Aa<span>EN</span></>}</div><div><strong>{extracting ? phaseTitle : "让我们听懂这段英语"}</strong><p>{extracting ? phaseDetail : "系统会处理你刚刚确认的选区或完整音频，识别完成后可直接校对。"}</p>{extracting && <div className="progress-track"><i /></div>}</div><button className="primary-button extract-button" type="button" onClick={extractContent} disabled={extracting}>{extracting ? <><i className="button-spinner" /> 任务进行中</> : <>开始提取英文 <b>→</b></>}</button></div>}
        {(stt.error || localError) && <div className="error-message" role="alert">!&nbsp; {stt.error || localError}</div>}
        {extractionDone && <div className="transcript-workspace"><div className="transcript-toolbar"><div><span className="lang-pill">EN</span><strong>English transcript</strong><small>{transcript.trim().split(/\s+/).filter(Boolean).length} words · 可直接编辑校对</small></div><div><button type="button" onClick={copyTranscript}>{copied ? "已复制 ✓" : "复制文本"}</button><a href={downloadUrl} download={transcriptName(clip.name)}>↓ 下载 TXT</a></div></div><textarea aria-label="英文识别内容" value={transcript} onChange={(event) => updateTranscript(event.target.value)} spellCheck="true" /><div className="transcript-foot"><span>提示：建议在进入翻译前校对人名、地名和专业词汇。</span><button type="button" onClick={extractContent} disabled={extracting}>↻ 重新提取</button></div></div>}
      </>}
    </section>
