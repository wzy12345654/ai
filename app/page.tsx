    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <div className="editor-area">
          {showLibrary && <div className="library-panel"><div className="library-head"><div><span className="section-kicker">MY WORKS</span><h2>我的作品</h2></div><button className="outline-button" onClick={() => setShowLibrary(false)}>返回创作台</button></div>{savedArticles.length ? savedArticles.map((article) => <button className="library-item" key={article.id} onClick={() => loadArticle(article)}><span><strong>{article.title}</strong><small>更新于 {new Date(article.updatedAt).toLocaleString("zh-CN")}</small></span><span>打开 →</span></button>) : <div className="empty-panel"><span>✦</span><p>还没有保存的作品，完成一次创作后保存吧。</p></div>}</div>}
          {showArticle && !showLibrary && <div className="article-panel"><div className="article-toolbar"><div><span className="section-kicker">FULL ARTICLE</span><h2>完整文章</h2><p>标题、正文、封面与配图已组合完成。</p></div><div><button className="outline-button" onClick={() => setShowArticle(false)}>返回编辑</button><button className="save-button" onClick={saveArticle}>{saveState === "尚未保存" ? "保存作品" : saveState}</button></div></div><article className="article-preview"><h1>{currentTitle}</h1>{latestCovers.length > 0 && <div className="article-cover-grid">{latestCovers.map((url, i) => <img key={`${url}-${i}`} src={url} alt={`封面 ${i + 1}`} />)}</div>}<div className="article-copy">{copy.split("\n").map((line, i) => line ? <p key={i}>{line}</p> : <br key={i} />)}</div>{bodyImages.length > 0 && <div className="article-body-images">{bodyImages.map((url, i) => <img key={`${url}-${i}`} src={url} alt={`正文配图 ${i + 1}`} />)}</div>}</article></div>}
          {!showArticle && !showLibrary && <>
          ...
          {activeTab === "cover" && <div className="visual-panel"><div className="style-row"><span className="section-kicker">封面风格</span>{styles.map((s) => <button key={s} className={style === s ? "style-active" : ""} onClick={() => setStyle(s)}>{s}</button>)}</div><div className="style-row ratio-row"><span className="section-kicker">比例</span><button className={coverRatio === "9:16" ? "style-active" : ""} onClick={() => setCoverRatio("9:16")}>9:16 竖版</button><button className={coverRatio === "4:3" ? "style-active" : ""} onClick={() => setCoverRatio("4:3")}>4:3 横版</button></div><div className="generate-row"><div><strong>生成 2 张封面</strong><small>每次生成都会保留历史结果 · 默认 1K · 低质量</small></div><button className="generate-image" onClick={generateCover} disabled={imageLoading}><span className="sparkle">✦</span>{imageLoading ? "正在生成…" : "开始生成封面"}<span>→</span></button></div>{imageError && <p className="error-text">{imageError}</p>}{coverHistory.length === 0 ? <div className="cover-preview"><span>✦</span><p>选择风格后，生成两张不同构图的封面</p><small>确认风格与比例后开始生成</small></div> : <div className="cover-history">{coverHistory.map((batch) => <div className="cover-batch" key={batch.id}><div className="batch-head"><span>{batch.style} · {batch.ratio}</span><small>本次生成 {batch.images.length} 张</small></div><div className="cover-grid">{batch.images.map((url, index) => <img key={`${batch.id}-${index}`} src={url} alt={`第 ${index + 1} 张封面`} />)}</div><button className="next-step-button" onClick={() => setActiveTab("images")}>用这组封面继续生成正文配图 →</button></div>)}</div>}</div>}
          {activeTab === "images" && <div className="visual-panel"><div className="panel-head"><div><span className="section-kicker">STEP 03 / BODY IMAGES</span><h3>让正文里的画面，也被看见。</h3></div><button className="outline-button" onClick={() => setActiveTab("cover")}>← 返回封面</button></div><p className="image-helper">已使用标题与全文正文作为生图提示词，可选择张数、比例与分辨率。</p><div className="image-options"><label>张数<select value={bodyCount} onChange={(e) => setBodyCount(e.target.value)}><option value="1">1 张</option><option value="3">3 张</option><option value="5">5 张</option></select></label><label>比例<select value={bodyRatio} onChange={(e) => setBodyRatio(e.target.value)}><option>1:1</option><option>4:3</option><option>9:16</option></select></label><label>分辨率<select value={bodyResolution} onChange={(e) => setBodyResolution(e.target.value)}><option>1K</option><option>2K</option></select></label><button className="generate-image" onClick={generateBodyImages} disabled={imageLoading}>{imageLoading ? "正在生成…" : "生成正文配图 →"}</button></div>{bodyImages.length > 0 ? <div className="body-image-grid">{bodyImages.map((url, index) => <img key={`${url}-${index}`} src={url} alt={`正文配图 ${index + 1}`} />)}</div> : <div className="empty-panel"><span>✦</span><p>设置参数后，生成与正文呼应的配图。</p></div>}</div>}
        </div>
      </section>
